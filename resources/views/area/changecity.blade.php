...
text = File.read("areas.txt")

File.open("areas.txt").each do |line|
  if (/href=\"(.+)\".*>(.+)<\/a>/ =~ line)
    x = $2.clone
  	if city = cities.find{|c| c[1].sub("市", "") == x}
      line2 = "\t\t\t\t\t\t<a href='#{city[0]}'>#{x}</a>\n"
      p line
      p line2
      text.gsub!(line, line2)
  	end
  end
end


File.open("areas2.txt", "w") {|file| file.puts text }
